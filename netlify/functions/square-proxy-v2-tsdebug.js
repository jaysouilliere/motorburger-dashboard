...
exports.handler = async function (event) {
  ...
  async function getTripleseatAccessToken() {
    ...
    const data = await r.json();
    if (!r.ok) throw new Error(`Tripleseat token refresh ${r.status}: ${JSON.stringify(data).substring(0,200)}`);
  ...
  async function getTripleseatEvents() {
    ...
    try {
      ...
      const today = new Date();
      const end = new Date(today); end.setDate(end.getDate()+7);
      let url = `${TS_EVENTS_URL}?event_start_date=${mmddyyyy(today)}&event_end_date=${mmddyyyy(end)}`;
      if (TS_LOCATION_ID) url += `&location_ids=${TS_LOCATION_ID}`;
      ...
      const data = await r.json();
      // API has returned either {events:[...]} or a bare array depending on doc version — handle both.
      ...
      const events = list
        ...
        .map(e => {
          ...
          return {
            ...
            venue: room ? `DSC — ${room}` : 'Detroit Shipping Company',
            url: '',
            category: 'Private Event',
            source: 'tripleseat',
            status: e.status || null,
            guestCount: e.guest_count ?? e.guaranteed_guest_count ?? null,
            account: (e.account && e.account.name) || '',
          };
        });
      return { events, error: null };
    } catch(e) { return { events: [], error: e.message }; }
  ...
  async function getTripleseatDiag() {
    ...
    try {
      ...
      const today = new Date();
      const end = new Date(today); end.setDate(end.getDate()+7);
      const eventsUrl = `${TS_EVENTS_URL}?event_start_date=${mmddyyyy(today)}&event_end_date=${mmddyyyy(end)}`;
