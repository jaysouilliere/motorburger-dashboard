...
## Active Bugs
- 🟢 **RESOLVED this session — `netlify/functions/staff-submit.js` restored.** Recovered from commit `c713cbb` (last version before deletion in `472cbbc`). Content verified matching the `/.netlify/functions/staff-submit` call in `staff-portal.html`, writes to blob store `motorburger-closeouts` (keys `latest` and `history-YYYY-MM-DD`). **⚠️ Still needs: commit the restored file to GitHub and confirm it appears in the deploy log's "Functions bundling" section (see Deploy Gotcha above — files can silently fail to bundle), then do a live test close-out submission and confirm it shows up in the Prep tab.**
- 🟢 **RESOLVED this session — silent-failure bug in `staff-portal.html` close-out submission.** Root cause of Brian's clock-out data vanishing: `submitCloseout()` had a `catch` block that fell back to `localStorage` on any fetch failure but then *still advanced to the "✅ Success" screen* — so staff had no way to know a submission never reached the server. This was masking the missing `staff-submit.js` the whole time and will mask any future outage the same way. Fixed: failed submissions now show a visible red error banner ("Submission failed — not saved to the server") instead of a fake success, keep the payload queued in `localStorage`, and auto-retry once on next page load via new `retryPendingCloseout()`.
...
- `netlify/functions/staff-submit.js` — restored (was missing)
- `netlify/functions/square-proxy.js` — net-vs-gross revenue fix + new `?debugDate=` diagnostic mode
- `staff-portal.html` — removed silent-failure fallback in `submitCloseout()`, added visible error state + auto-retry
**To deploy:** upload these three files to their same paths in the GitHub repo, commit, redeploy, then check the deploy log's "Functions bundling" section per the Deploy Gotcha note — don't assume a clean deploy just because there's no error.
...
6. 🔴 **NEW TOP PRIORITY — Deploy this session's fixes.** Three files staged in outputs (staff-submit.js restored, square-proxy.js net-revenue fix + debug endpoint, staff-portal.html error-visibility fix) need to be committed to GitHub and redeployed, then verified live: (a) confirm staff-submit.js bundles in the deploy log, (b) do a real close-out submission and confirm it appears in the Prep tab, (c) confirm the Prep tab shows 27 proteins for Wed 07-01 and revenue matching Square's Net Sales report.
