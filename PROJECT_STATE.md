...
- `staff-portal.html` — geofence fix, **committed to GitHub `main` this session.** Clock-in now blocks (not silently allows) when location is denied; clock-out now enforces the same on-site geofence check for the first time (it had none before); manager PIN override available for both, tagged in Timesheets view.
**Note:** last session's three files (`staff-submit.js`, `square-proxy.js`, and the earlier `staff-portal.html` error-visibility fix) were confirmed already live on GitHub `main` this session — see Active Bugs.
**Still needs:** confirm this commit appears clean in the Netlify deploy log, then a live phone test of the geofence override flow (see Active Bugs for exact test steps).
