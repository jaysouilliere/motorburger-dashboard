...
netlify/functions/
  square-proxy.js       Square + Ticketmaster + Masonic scraper + blob reader + ?tips=true (per-day Square tip pull, added this session)
  ...
  shifts.js              NEW this session — server-side shift storage (motorburger-shifts blob), replaces localStorage-only clock data. Actions: GET (list), POST save/update/bulkUpsert.
  tips-override.js       NEW this session — per-day manual tip-pool overrides (motorburger-tips blob). Actions: GET, POST {date,amount} or {action:'bulkSet', entries}.
```
**⚠️ Deploy gotcha (recurring — bit us three times now):** Files placed in the wrong spot, or never actually committed, deploy silently with no error — they just don't work, and nothing in the Netlify UI flags it.
...
- `staff-portal.html` — major rewrite, see Pay Period section below for full detail. Also: shift storage moved from localStorage-only to server-backed (Blob), Team tab gained a historical-CSV import tool, new Manual Entry tab added.
- `netlify/functions/square-proxy.js` — added `?tips=true&start=&end=` action, pulls per-day tip totals from Square's Payments API (`tip_money`). This didn't exist before — Orders API never carried tips, so nothing in this file had ever fetched them.
- `netlify/functions/shifts.js` — **NEW.** Server-side shift storage (see File Structure).
- `netlify/functions/tips-override.js` — **NEW.** Per-day manual tip-pool override storage (see File Structure).
- Prior session's `staff-submit.js` / geofence work — unchanged this session, still per Active Bugs above (manager-override-on-phone bug still unconfirmed/undiagnosed).
...
- **Historical data import** (Team tab → "Import Historical Data"): paste old tracker CSV/TSV rows, Preview shows row count/date range/employee matches before writing anything, Import writes via `bulkUpsert` (id = date+staff+role, so re-pasting corrected data overwrites instead of duplicating) plus a `bulkSet` of daily gratuity totals as overrides. Employee names matched to current roster by first name (so "Brian" merges into "brian"/"Brian Rockwood"); unmatched names (e.g. former employees) kept under their own slugified id so history isn't lost. **Known limitation:** imported rows have `rate:0` (old sheet didn't track hourly rate) — hours/tips are accurate but wages will show $0 for historical dates; don't run wage totals across a range spanning old + new data.
- **Manual Entry tab** (bridge until staff use the in-app clock regularly): pick a date, type in Square's tip total + each person's hours (from Homebase), Save Day. Same `bulkUpsert`/id scheme as the import, so re-saving a date overwrites instead of duplicating — this is how Jay corrects a wrong entry (e.g. Jul 1 tip total mistake, fixed this session by re-saving that date).
- **Jay has NOT yet imported the ~2 years of historical CSV data** (starts ~Aug 2025 per Jay, though the exact start date didn't perfectly reconcile with "almost 2 years" — not corrected, not important). He has the Apps Script code and old sheet; import is ready whenever he pastes it in.
- Live-tested working end to end this session: Manual Entry save (after fixing the missing-functions deploy gotcha above), per-day audit/edit expand-and-fix flow.
...
10. ✅ DONE this session — Pay Period tab overhauled top to bottom. See dedicated section below for full detail.
