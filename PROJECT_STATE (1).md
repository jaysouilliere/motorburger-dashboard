...
```
**⚠️ Deploy gotcha (bit us this session):** `staff-portal.html` was accidentally uploaded into `netlify/functions/` instead of repo root — it sat there invisible to the live site while Jay kept redeploying and seeing no changes. When editing `staff-portal.html` or `index.html`, always confirm the breadcrumb reads `motorburger-dashboard / staff-portal.html`, NOT `motorburger-dashboard / netlify / functions / staff-portal.html`.
...
**Recovered incident:** `netlify/functions/staff-submit.js` was accidentally deleted (commit `472cbbc`) during the misplaced-file cleanup above. Restored from git history same session — confirmed working again, but worth a spot-check that close-out submissions are still saving to blob storage correctly.
...
3. ✅ DONE — Fixed date/timezone bug in index.html: `dateStr()` was using UTC (`toISOString()`) instead of Detroit local time, causing "Today"/"Tomorrow" event buckets to shift incorrectly between 8pm–midnight Detroit time (UTC rolls to next day but Detroit hasn't yet). Now matches staff-portal.html's Detroit-local approach. Affected Events tab AND Yesterday/Prep tabs (same function).
4. ✅ DONE — Removed wage/rate visibility from staff-facing screens (see Staff Portal section above)
5. 🔶 IN PROGRESS — Tripleseat integration (DSC booked events: private parties, pedal pubs, etc.) — see dedicated section below. Currently blocked on Jay getting Tripleseat customer admin role to create OAuth app.
6. Remove Alberto, add new hire (dual role)
7. On-site PIN/geofence test at DSC
8. 🔴911 urgent restock flag system (single system — see Prep List spec below, do not build twice)
9. Auto pull tip pool from Square (currently manual entry in Pay Period tab)
10. Build closing checklist out properly on-site
11. Masonic capacity buffer (main theatre 4900cap +35%, small rooms +15%)
12. Repo cleanup: delete staff-portal(old).html, files.zip
13. Consider: should staff portal also show a "tomorrow" events heads-up, not just today? (came up this session re: Ford Field, undecided)
